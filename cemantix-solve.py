...
def setup_driver(no_ui: bool, website_name: str):
    if no_ui:
...
def main(model: KeyedVectors, no_ui: bool,cemantle: bool, threshold: float = 0.06):
    website_name = "cemantix" if not cemantle else "cemantle"
    driver = setup_driver(no_ui, website_name)
...
if __name__ == '__main__':
    possible_args = ["--help", "--reddit", "--no-ui", "--vector-file", "--cemantle"]
...
--vector-file : Select the name of the pretrained word2vec file you want to use (default is frWac_no_postag_no_phrase_700_skip_cut50.bin)""")
    ...
    if possible_args[1] in arguments:
        test_reddit_creds()
    words = main(model=vector_model, no_ui=possible_args[2] in arguments, threshold=0.06,cemantle=possible_args[4] in arguments)
    if possible_args[1] in arguments:
